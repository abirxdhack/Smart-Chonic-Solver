...
export const Route = createFileRoute("/parabola")({
  ...
  head: () => ({
    meta: [
      { title: "পরাবৃত্ত সমাধান ও গ্রাফ — Conic Studio" },
      {
        name: "description",
        content:
          "পরাবৃত্তের যেকোন সমীকরণ বা প্যারামিটার থেকে শীর্ষ, উপকেন্দ্র, নিয়ামক, লম্ব ও গ্রাফ; ধাপে ধাপে ব্যাখ্যা।",
      },
      { property: "og:title", content: "পরাবৃত্ত সমাধান" },
      { property: "og:description", content: "ইন্টারেক্টিভ পরাবৃত্ত গ্রাফ ও ধাপে ধাপে সমাধান।" },
...
function ParabolaPage() {
  ...
  const eqDetail = useMemo(() => (tab === "eq" && eq.trim() ? parseParabolaDetailed(eq) : null), [eq, tab]);
  useEffect(() => {
    if (tab === "eq") {
      ...
      <main className="page-main">
        <div className="grid-layout">
          <section className="card input-card fade-in" id="input">
            ...
            <div className="tabs">
              <button className={`tab ${tab === "param" ? "active" : ""}`} onClick={() => setTab("param")}>
                প্যারামিটার
              </button>
              <button className={`tab ${tab === "eq" ? "active" : ""}`} onClick={() => setTab("eq")}>
                সমীকরণ
              </button>
            </div>
            ...
            {tab === "param" ? (
              <div className="param-panel">
                <div className="orient-toggle">
                  <span className="lbl">অক্ষরেখা</span>
                  <div className="seg">
                    <button className={orientation === "x" ? "on" : ""} onClick={() => setOrientation("x")}>
                      x-অক্ষের সমান্তরাল
                    </button>
                    <button className={orientation === "y" ? "on" : ""} onClick={() => setOrientation("y")}>
                      y-অক্ষের সমান্তরাল
                    </button>
                  </div>
                </div>
                <div className="fields">
                  <label className="fld">
                    <span>h (শীর্ষ x)</span>
                    <input type="text" inputMode="decimal" placeholder="h" value={hIn} onChange={(e) => setHIn(e.target.value)} />
                  </label>
                  <label className="fld">
                    <span>k (শীর্ষ y)</span>
                    <input type="text" inputMode="decimal" placeholder="k" value={kIn} onChange={(e) => setKIn(e.target.value)} />
                  </label>
                  <label className="fld">
                    <span>a</span>
                    <input type="text" inputMode="decimal" placeholder="a" value={aIn} onChange={(e) => setAIn(e.target.value)} />
                  </label>
                </div>
              ...
              <div className="eq-panel">
                ...
                <div className="hint">
                  উদাহরণ: <code>(y+3)^2=16(x-2)</code> বা <code>(x-1)^2=8(y+4)</code>
                </div>
                {kbOpen && (
                  <MathKeyboard
            ...
            <div style={{ marginTop: 16 }}>
              <div className="orient-toggle">
                ...
                <div className="fields" style={{ marginTop: 4 }}>
                  <label className="fld">
                    <span>x</span>
                    <input type="text" inputMode="decimal" placeholder="x" value={pxIn} onChange={(e) => setPxIn(e.target.value)} />
                  </label>
                  <label className="fld">
                    <span>y</span>
                    <input type="text" inputMode="decimal" placeholder="y" value={pyIn} onChange={(e) => setPyIn(e.target.value)} />
                  </label>
                </div>
              </div>
            </div>
          ...
          <section className="card graph-card fade-in" ref={svgWrapRef} id="graph">
            <div className="graph-head">
              ...
              <div className="graph-tools">
                <span className="chip"><span className="dot" style={{ background: CONIC_COLORS.curve }} />পরাবৃত্ত</span>
                <span className="chip"><span className="dot" style={{ background: CONIC_COLORS.vertex }} />শীর্ষ</span>
                <span className="chip"><span className="dot" style={{ background: CONIC_COLORS.focus }} />উপকেন্দ্র</span>
                <span className="chip"><span className="dot" style={{ background: CONIC_COLORS.directrix }} />নিয়ামক</span>
              </div>
            </div>
            {parsed ? <ParabolaGraphInner parsed={parsed} /> : <div className="err soft">সঠিক ইনপুট দিন</div>}
          </section>
          ...
          <section className="card results-card fade-in" id="results">
            ...
            {parsed ? (
              ...
              <div className="err soft">সঠিক ইনপুট দিন — পরাবৃত্ত তৈরি হয়নি।</div>
            )}
          </section>
        </div>
      </main>
...
function ParabolaGraphInner({ parsed }: { parsed: ParabolaParsed }) {
  ...
  const guides: Guide[] = [
    ...
    { kind: "segment", x1: geo.latusEnds[0][0], y1: geo.latusEnds[0][1], x2: geo.latusEnds[1][0], y2: geo.latusEnds[1][1], color: CONIC_COLORS.latus, width: 2.4 },
  ];
  const markers: Marker[] = [
    { x: geo.vertex[0], y: geo.vertex[1], color: CONIC_COLORS.vertex, name: "শীর্ষবিন্দু", label: fmtPt(geo.vertex), placement: "above-left" },
    { x: geo.focus[0], y: geo.focus[1], color: CONIC_COLORS.focus, name: "উপকেন্দ্র", label: fmtPt(geo.focus), placement: "below-right" },
    { x: geo.dirFoot[0], y: geo.dirFoot[1], color: CONIC_COLORS.dirFoot, name: "নিয়ামক পাদবিন্দু", label: fmtPt(geo.dirFoot), placement: "below-left" },
    { x: geo.latusEnds[0][0], y: geo.latusEnds[0][1], color: CONIC_COLORS.latusEnd, name: "উপকেন্দ্রিক লম্বের প্রান্তবিন্দু", label: fmtPt(geo.latusEnds[0]), placement: "above-right" },
    { x: geo.latusEnds[1][0], y: geo.latusEnds[1][1], color: CONIC_COLORS.latusEnd, name: "উপকেন্দ্রিক লম্বের প্রান্তবিন্দু", label: fmtPt(geo.latusEnds[1]), placement: "below-right" },
  ];
  const curves =
    orientation === "x"
      ? [{ type: "xOfY" as const, fn: (y: number) => h + ((y - k) ** 2) / (4 * a) }]
      : [{ type: "yOfX" as const, fn: (x: number) => k + ((x - h) ** 2) / (4 * a) }];
  return <ConicGraph bounds={bounds} curves={curves} guides={guides} markers={markers} ariaLabel="পরাবৃত্ত গ্রাফ" />;
}

function ParabolaResults({
  parsed,
  px,
  py,
}: {
  parsed: ParabolaParsed;
  px: number;
  py: number;
}) {
  const { h, k, a, orientation } = parsed;
  ...
  const rows = [
    {
      ...
      steps: [
        ...
        `এখানে h = ${toBn(h)}, k = ${toBn(k)}`,
        `∴ শীর্ষবিন্দু = (h, k) = ${fmtPt(geo.vertex)}`,
      ],
    },
    {
    ...
    },
    {
      ...
      v: fmtPt(geo.dirFoot),
      steps: [
        orientation === "x" ? `পাদবিন্দু = (h − a, k)` : `পাদবিন্দু = (h, k − a)`,
        `∴ = ${fmtPt(geo.dirFoot)}`,
      ],
    },
    {
      c: CONIC_COLORS.latusEnd,
      l: "উপকেন্দ্রিক লম্বের প্রান্তবিন্দুদ্বয়",
      v: `${fmtPt(geo.latusEnds[0])} ও ${fmtPt(geo.latusEnds[1])}`,
      steps: [
        `উপকেন্দ্র দিয়ে অক্ষের লম্ব রেখা প্যারাবোলাকে যেখানে ছেদ করে`,
        orientation === "x"
          ? `x = h + a বসিয়ে (y − k)² = 4a·a ⇒ y = k ± 2|a|`
          : `y = k + a বসিয়ে (x − h)² = 4a·a ⇒ x = h ± 2|a|`,
        `∴ প্রান্তবিন্দু = ${fmtPt(geo.latusEnds[0])} ও ${fmtPt(geo.latusEnds[1])}`,
      ],
    },
    {
      c: CONIC_COLORS.axis,
      ...
      v: axisEq,
      steps: [
        orientation === "x" ? `অক্ষ শীর্ষ দিয়ে অতিক্রম করে x-অক্ষের সমান্তরাল` : `অক্ষ শীর্ষ দিয়ে অতিক্রম করে y-অক্ষের সমান্তরাল`,
        `∴ ${axisEq}`,
      ],
    },
    {
      c: CONIC_COLORS.latus,
      l: "উপকেন্দ্রিক লম্বের সমীকরণ",
      v: latusEq,
      steps: [`উপকেন্দ্র বিন্দু দিয়ে অক্ষের লম্ব রেখা`, `∴ ${latusEq}`],
    },
    {
      ...
      v: dirEq,
      steps: [
        orientation === "x" ? `নিয়ামক x = h − a` : `নিয়ামক y = k − a`,
        `∴ ${dirEq}`,
      ],
    },
    {
      c: CONIC_COLORS.tangent,
      l: "শীর্ষবিন্দুতে স্পর্শকের সমীকরণ",
      v: tangentEq,
      steps: [
        `শীর্ষে অক্ষের লম্ব হলো স্পর্শক`,
        `∴ ${tangentEq}`,
      ],
    },
    {
      c: CONIC_COLORS.latus,
      l: "উপকেন্দ্রিক লম্বের দৈর্ঘ্য",
      v: `|৪a| = ${toBn(Math.abs(4 * a))}`,
      steps: [`দৈর্ঘ্য = |4a| = ${toBn(Math.abs(4 * a))} একক`],
    },
    {
      c: "#22d3ee",
