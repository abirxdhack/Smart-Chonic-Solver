...
export const Route = createFileRoute("/ellipse")({
  ...
  head: () => ({
    meta: [
      { title: trs("উপবৃত্ত সমাধান ও গ্রাফ — Conic Studio") },
      {
        name: "description",
        content: trs(
      ...
      { property: "og:title", content: trs("উপবৃত্ত সমাধান") },
      {
        property: "og:description",
        content: trs("ইন্টারেক্টিভ উপবৃত্ত গ্রাফ ও ধাপে ধাপে সমাধান।"),
      },
...
function EllipsePage() {
  ...
  const eqDetail = useMemo(
    () => (tab === "eq" && eq.trim() ? parseEllipseDetailed(eq) : null),
    [eq, tab],
  );
  useEffect(() => {
    if (tab === "eq") {
      ...
      <main className="page-main">
        <div className="grid-layout">
            ...
            {tab === "param" ? (
              <div>
                ...
                <div className="preview-eq">
                  ...
                  <code>
                    (x {alpha >= 0 ? "−" : "+"} {toBn(Math.abs(alpha))})² / {toBn(a * a)} + (y{" "}
                    {beta >= 0 ? "−" : "+"} {toBn(Math.abs(beta))})² / {toBn(b * b)} {trs("= ১")}
                  </code>
                </div>
              </div>
            ) : (
              <div>
                ...
                <div className="hint">
                  {trs("উদাহরণ:")} <code>(x-2)^2/25+(y+1)^2/9=1</code>
                </div>
                {kbOpen && (
                  <MathKeyboard
                    onInsert={insertAt}
          ...
          <section className="card graph-card fade-in" ref={graphRef}>
            <div className="graph-head">
              ...
              <div className="graph-tools">
                ...
                <span className="chip">
                  <span className="dot" style={{ background: CONIC_COLORS.center }} />
                  {trs("কেন্দ্র")}
                </span>
                <span className="chip">
            ...
            {parsed ? (
              <EllipseGraphInner parsed={parsed} />
            ) : (
          ...
          <section className="card fade-in">
            ...
            {parsed ? (
              <EllipseResults parsed={parsed} px={px} py={py} />
            ) : (
              <div className="err soft">{trs("সঠিক ইনপুট দিন।")}</div>
            )}
          </section>
  ...
  const curves = [
    {
      ...
      sample: (t: number) =>
        (horiz
          ...
          : [parsed.alpha + parsed.b * Math.cos(t), parsed.beta + parsed.a * Math.sin(t)]) as [
          number,
          number,
        ],
      tStart: 0,
  ...
  const rows = [
    {
      ...
      l: trs("কেন্দ্র"),
      v: fmtPt(geo.center),
      steps: [trt`কেন্দ্র (α, β) = (${toBn(alpha)}, ${toBn(beta)})`],
    },
    {
      c: "#22d3ee",
    ...
    {
      c: CONIC_COLORS.axis,
      l: trs("বৃহৎ অক্ষের দৈর্ঘ্য"),
      v: `${toBn(geo.majorLen)}`,
      steps: [`2a = 2·${toBn(a)} = ${toBn(geo.majorLen)}`],
    },
    {
      c: CONIC_COLORS.minor,
      l: trs("ক্ষুদ্র অক্ষের দৈর্ঘ্য"),
      v: `${toBn(geo.minorLen)}`,
      steps: [`2b = 2·${toBn(b)} = ${toBn(geo.minorLen)}`],
    },
    {
      c: CONIC_COLORS.axis,
      l: trs("বৃহৎ অক্ষের সমীকরণ"),
      v: geo.majorAxisEq,
      steps: [horiz ? "y = β" : "x = α"],
    },
    {
      c: CONIC_COLORS.aux,
      l: trs("ক্ষুদ্র অক্ষের সমীকরণ"),
      v: geo.minorAxisEq,
      steps: [horiz ? "x = α" : "y = β"],
    },
    {
      ...
      steps: [horiz ? "(α ± a, β)" : "(α, β ± a)"],
    },
    {
      c: CONIC_COLORS.focus,
      l: trs("ফোকাসদ্বয়"),
      v: trt`${fmtPt(geo.foci[0])} ও ${fmtPt(geo.foci[1])}`,
      steps: [`c = ae = ${fmtNum(geo.c)}`, horiz ? "(α ± c, β)" : "(α, β ± c)"],
    },
    {
      c: CONIC_COLORS.focus,
    ...
    },
    {
      ...
      steps: [horiz ? "(α ± a/e, β)" : "(α, β ± a/e)"],
    },
    {
      c: CONIC_COLORS.directrix,
      l: trs("নিয়ামক দুটির দূরত্ব"),
      v: trt`২a/e = ${fmtNum(geo.directricesDist)}`,
      steps: [`= 2a/e = 2·${toBn(a)}/${fmtNum(geo.e)} = ${fmtNum(geo.directricesDist)}`],
    },
    {
      c: CONIC_COLORS.directrix,
      l: trs("নিয়ামকের সমীকরণ"),
      v: trt`${geo.directrixEqs[0]}  ও  ${geo.directrixEqs[1]}`,
      steps: [horiz ? "x = α ± a/e" : "y = β ± a/e"],
    },
    {
      c: CONIC_COLORS.latus,
      l: trs("উপকেন্দ্রিক লম্বের দৈর্ঘ্য"),
      v: fmtNum(geo.latusRectumLen),
      steps: [`= 2b²/a = 2·${toBn(b * b)}/${toBn(a)} = ${fmtNum(geo.latusRectumLen)}`],
    },
    {
      c: CONIC_COLORS.latus,
      l: trs("উপকেন্দ্রিক লম্বের সমীকরণ"),
      v: trt`${geo.latusRectumEqs[0]}  ও  ${geo.latusRectumEqs[1]}`,
      steps: [trs("ফোকাস দিয়ে বৃহৎ অক্ষের লম্ব")],
    },
    {
      c: "#22d3ee",
      l: trt`(${toBn(px)}, ${toBn(py)}) বিন্দুর উপকেন্দ্রিক দূরত্ব`,
      v: trt`S₁P = ${fmtNum(fd.d1)}, S₂P = ${fmtNum(fd.d2)}, যোগফল = ${fmtNum(fd.sum)}`,
      steps: [
        `S₁P = √((x − x_{S₁})² + (y − y_{S₁})²) = ${fmtNum(fd.d1)}`,
        `S₂P = ${fmtNum(fd.d2)}`,
        trt`উপবৃত্তের ধর্ম অনুযায়ী S₁P + S₂P = 2a = ${toBn(2 * a)}`,
        trt`প্রাপ্ত যোগফল = ${fmtNum(fd.sum)}`,
      ],
    },
    {
      c: "#f472b6",
      ...
      v: posText,
      steps: [trt`(x−α)²/A² + (y−β)²/B² − 1 এর মান দিয়ে যাচাই`],
    },
  ];
  return (
