...
export function ConicGraph({
  ...
  const minorX: number[] = [];
  const minorY: number[] = [];
  for (let x = Math.ceil(bounds.xmin / step) * step; x <= bounds.xmax + 1e-9; x += step)
    majorX.push(+x.toFixed(4));
  for (let y = Math.ceil(bounds.ymin / step) * step; y <= bounds.ymax + 1e-9; y += step)
    majorY.push(+y.toFixed(4));
  ...
  const originInView =
    showOrigin && bounds.xmin <= 0 && bounds.xmax >= 0 && bounds.ymin <= 0 && bounds.ymax >= 0;
    ...
    for (const m of fullMarkers) {
      ...
      for (const off of offsets) {
        for (const p of ordered) {
          ...
          const rx = tx - 3;
          const ry = ty - boxH + 6;
          if (rx < pad + 2 || rx + w > W - pad - 2 || ry < pad + 2 || ry + h > H - pad - 2)
            continue;
  ...
  return (
    <div className="graph-wrap">
      ...
      >
        ...
        <text
          x={W - pad - 6}
          y={(bounds.ymin <= 0 && bounds.ymax >= 0 ? sy(0) : pad + 16) - 8}
          className="axis-name"
          textAnchor="end"
        >
          x
        ...
        <text
          x={(bounds.xmin <= 0 && bounds.xmax >= 0 ? sx(0) : W - pad - 16) + 8}
          y={pad + 14}
          className="axis-name"
        >
          y
