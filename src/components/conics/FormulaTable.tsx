...
import { localDigits } from "@/lib/conics/format";
    ...
    <div className={`ftable-wrap accent-${accent}`}>
      <div className="ftable-scroll">
        <table className="ftable">
          <thead>
            <tr>
              <th className="col-idx">{trs("ক্রম")}</th>
              <th className="col-name">{trs("উপাদানের নাম")}</th>
              {headers.map((h, i) => (
                <th key={i}>{h}</th>
              ))}
            </tr>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} style={{ animationDelay: `${i * 45}ms` }}>
                ...
                <td className="col-name">{trs(r.label)}</td>
                {r.cells.map((c, j) => (
                  <td key={j}>
