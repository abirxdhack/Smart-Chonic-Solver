  ...
  const model = useMemo(() => {
    ...
    if (solution.kind === "parabola") {
      ...
      if (orientation === "x") {
        ...
        guides.push({ kind: "vline", x: alpha - A, color: ADV_COLORS.directrix, dash: "9 6", width: 2.4 });
      } else {
        guides.push({ kind: "vline", x: alpha, color: ADV_COLORS.axis, dash: "3 5", width: 1.8 });
        guides.push({ kind: "hline", y: beta - A, color: ADV_COLORS.directrix, dash: "9 6", width: 2.4 });
      }
      guides.push({
        kind: "segment",
        x1: l1[0],
      ...
      markers.push({ x: alpha, y: beta, color: ADV_COLORS.vertex, name: "শীর্ষ", label: pt(alpha, beta), placement: "above-left" });
      markers.push({ x: focus[0], y: focus[1], color: ADV_COLORS.focus, name: "উপকেন্দ্র", label: pt(focus[0], focus[1]), placement: "below-right" });
      markers.push({ x: dirFoot[0], y: dirFoot[1], color: ADV_COLORS.directrix, name: "নিয়ামক পাদবিন্দু", label: pt(dirFoot[0], dirFoot[1]), placement: "below-left" });
      markers.push({ x: l1[0], y: l1[1], color: ADV_COLORS.latus, name: "লম্বের প্রান্ত", label: pt(l1[0], l1[1]), placement: "above-right" });
      markers.push({ x: l2[0], y: l2[1], color: ADV_COLORS.latus, name: "লম্বের প্রান্ত", label: pt(l2[0], l2[1]), placement: "below-right" });
      const T = Math.sqrt(Math.max(span / Math.max(abs, 1e-6), 1)) * 2 + 4;
      const sample = (t: number): [number, number] =>
        orientation === "x" ? [alpha + A * t * t, beta + 2 * A * t] : [alpha + 2 * A * t, beta + A * t * t];
      return {
        bounds,
        guides,
        markers,
    ...
    if (horiz) {
      ...
      guides.push({ kind: "vline", x: alpha - vA / e, color: ADV_COLORS.directrix, dash: "9 6", width: 2 });
      guides.push({ kind: "vline", x: alpha + vA / e, color: ADV_COLORS.directrix, dash: "9 6", width: 2 });
    } else {
      ...
      guides.push({ kind: "hline", y: beta - vA / e, color: ADV_COLORS.directrix, dash: "9 6", width: 2 });
      guides.push({ kind: "hline", y: beta + vA / e, color: ADV_COLORS.directrix, dash: "9 6", width: 2 });
    }
    if (!isEllipse) {
      const m = horiz ? b / a : a / b;
      guides.push({ kind: "line", a: m, b: -1, c: beta - m * alpha, color: ADV_COLORS.axis, dash: "6 6", width: 1.6 });
      guides.push({ kind: "line", a: -m, b: -1, c: beta + m * alpha, color: ADV_COLORS.axis, dash: "6 6", width: 1.6 });
    }
    markers.push({ x: alpha, y: beta, color: ADV_COLORS.vertex, name: "কেন্দ্র", label: pt(alpha, beta), placement: "above-left" });
    markers.push({ x: v1[0], y: v1[1], color: ADV_COLORS.curve, name: "শীর্ষ", label: pt(v1[0], v1[1]), placement: "below-left" });
    markers.push({ x: v2[0], y: v2[1], color: ADV_COLORS.curve, name: "শীর্ষ", label: pt(v2[0], v2[1]), placement: "above-right" });
    markers.push({ x: f1[0], y: f1[1], color: ADV_COLORS.focus, name: "ফোকাস", label: pt(f1[0], f1[1]), placement: "below-right" });
    markers.push({ x: f2[0], y: f2[1], color: ADV_COLORS.focus, name: "ফোকাস", label: pt(f2[0], f2[1]), placement: "above-right" });
        ...
        curves: [
          {
            ...
            sample: (t: number): [number, number] => [alpha + rx * Math.cos(t), beta + ry * Math.sin(t)],
            tStart: 0,
            tEnd: Math.PI * 2,
            n: 720,
          },
