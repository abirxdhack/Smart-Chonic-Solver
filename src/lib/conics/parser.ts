...
function mulP(a: Poly, b: Poly): Poly {
  ...
  const db = degP(b);
  if (da + db > 2) {
    throw new ParseException("সমীকরণের ঘাত ২-এর বেশি হতে পারবে না", "সমীকরণটি x² অথবা y² পর্যন্ত সীমাবদ্ধ রাখুন");
  }
  const out = zero();
  out.c = a.c * b.c;
...
function powP(base: Poly, exp: number): Poly {
  if (!Number.isInteger(exp) || exp < 0 || exp > 2) {
    throw new ParseException("ঘাত অবশ্যই 0, 1 বা 2 হতে হবে", "শুধুমাত্র বর্গ (^2) ব্যবহার করুন");
  }
...
function divP(a: Poly, b: Poly): Poly {
  if (degP(b) !== 0) {
    throw new ParseException("চলক দিয়ে ভাগ সমর্থিত নয়", "শুধু ধ্রুবক দিয়ে ভাগ ব্যবহার করুন");
  }
  if (Math.abs(b.c) < EPS) {
    throw new ParseException("শূন্য দিয়ে ভাগ", "হরে অশূন্য মান দিন");
  }
...
export function sanitize(raw: string): { text: string; changed: boolean } {
  const original = raw;
  let s = raw;
  s = s.replace(/[⁰¹²³⁴⁵⁶⁷⁸⁹]+/g, (m) => "^" + m.split("").map((c) => SUP_MAP[c]).join(""));
  s = s.replace(/[−–—]/g, "-");
  s = s.replace(/[×·⋅]/g, "*");
...
function tokenize(s: string): Tok[] {
  ...
  while (i < s.length) {
    ...
    if (ch === "x" || ch === "y") {
      out.push({ t: "var", v: ch }); i++; continue;
    }
    if (/[0-9.]/.test(ch)) {
      let j = i;
      ...
      if (!isFinite(num)) {
        throw new ParseException(`অবস্থান ${i + 1}-এ অসংখ্যিক মান`, "শুধু বৈধ সংখ্যা ব্যবহার করুন");
      }
      out.push({ t: "num", v: num });
      i = j;
      continue;
    }
    throw new ParseException(`অবস্থান ${i + 1}-এ অপরিচিত অক্ষর: "${ch}"`, "কেবল x, y, সংখ্যা, +, −, ×, /, ^, ( ) ব্যবহার করুন");
  }
  return out;
}
...
class Parser {
    ...
    if (!this.eof()) {
      ...
      if (p.t === "op" && p.v === "^") {
        ...
        if (!exTok || exTok.t !== "num") {
          throw new ParseException("ঘাতের পরে সংখ্যা থাকতে হবে", "যেমন x^2 লিখুন");
        }
  ...
  parseAtom(): Poly {
    if (this.eof()) throw new ParseException("সমীকরণ অসম্পূর্ণ", "শেষে চলক বা সংখ্যা প্রত্যাশিত");
    const p = this.eat();
    ...
    if (p.t === "lp") {
      ...
      if (!close || close.t !== "rp") {
        throw new ParseException("বন্ধনী মেলেনি", "প্রতিটি ( এর সাথে ) দিন");
      }
      return inner;
    }
    throw new ParseException("অপ্রত্যাশিত টোকেন", "সমীকরণের গঠন যাচাই করুন");
  }
}

function parseSide(s: string): Poly {
  if (!s) throw new ParseException("সমীকরণের একটি পাশ ফাঁকা", "= এর দুই পাশেই রাশি দিন");
  const toks = tokenize(s);
  if (toks.length === 0) throw new ParseException("সমীকরণের একটি পাশ ফাঁকা", "= এর দুই পাশেই রাশি দিন");
  const p = new Parser(toks);
  const val = p.parseExpr();
  if (!p.eof()) throw new ParseException("অতিরিক্ত টোকেন", "গঠন যাচাই করুন");
  return val;
}
...
function normalize(p: Poly): NormalizedConic {
  if (Math.abs(p.xy) > 1e-6) {
    throw new ParseException("এই অ্যাপে ঘূর্ণিত (xy) কণিক সমর্থিত নয়", "xy টার্ম বাদ দিন");
  }
  const A = p.xx;
  const C = p.yy;
  const D = p.x;
  ...
  if (aZero && cZero) {
    throw new ParseException("কণিক নয়: বর্গ পদ অনুপস্থিত", "x² বা y² অন্তত একটি রাখুন");
  }

  if (aZero || cZero) {
    if (aZero) {
      if (Math.abs(D) < EPS) {
        throw new ParseException("পরাবৃত্ত অবৈধ: রৈখিক x পদ অনুপস্থিত", "উদাহরণ: y² = 4x");
      }
      const k = -E / (2 * C);
      const a = -D / (4 * C);
    ...
    if (Math.abs(E) < EPS) {
      throw new ParseException("পরাবৃত্ত অবৈধ: রৈখিক y পদ অনুপস্থিত", "উদাহরণ: x² = 4y");
    }
    const h = -D / (2 * A);
    const a = -E / (4 * A);
  ...
  if (A * C > 0) {
    ...
    if (pA < EPS || pC < EPS) {
      if (pA < -EPS || pC < -EPS) {
        throw new ParseException("বাস্তব উপবৃত্ত নয়", "সহগ বা ধ্রুবকের চিহ্ন যাচাই করুন");
      }
      throw new ParseException("অধঃপতিত উপবৃত্ত (একটি বিন্দু)", "সহগ যাচাই করুন");
    }
    let orientation: "horizontal" | "vertical";
    let aSq: number, bSq: number;
    if (pA >= pC) { orientation = "horizontal"; aSq = pA; bSq = pC; }
    else { orientation = "vertical"; aSq = pC; bSq = pA; }
  ...
  if (Math.abs(pA) < EPS || Math.abs(pC) < EPS) {
    throw new ParseException("অধঃপতিত অধিবৃত্ত", "সহগ যাচাই করুন");
  }
  let orientation: "horizontal" | "vertical";
  let aSq: number, bSq: number;
  if (pA > 0) { orientation = "horizontal"; aSq = pA; bSq = -pC; }
  else { orientation = "vertical"; aSq = pC; bSq = -pA; }
  if (aSq <= EPS || bSq <= EPS) {
    throw new ParseException("বাস্তব অধিবৃত্ত নয়", "সহগ বা চিহ্ন যাচাই করুন");
  }
  const av = Math.sqrt(aSq);
  const bv = Math.sqrt(bSq);
  const standard = orientation === "horizontal"
...
export function analyzeConic(raw: string): ParseResult {
  ...
  if (!raw.trim()) {
    return { ok: false, error: { message: "সমীকরণ ফাঁকা", suggestion: "একটি সমীকরণ টাইপ করুন" }, sanitized: text, autoCorrected: false };
  }
  try {
    if (!text.includes("=")) {
      throw new ParseException("সমান চিহ্ন (=) অনুপস্থিত", "উদাহরণ: x²/16 + y²/9 = 1");
    }
    const parts = text.split("=");
    if (parts.length !== 2) {
      throw new ParseException("একের অধিক = চিহ্ন", "কেবল একটি = ব্যবহার করুন");
    }
    ...
    const err = e instanceof ParseException
      ? { message: e.message, suggestion: e.suggestion }
      : { message: "সমীকরণ পার্স করা যায়নি", suggestion: "গঠন যাচাই করুন" };
