...
export function parseLine(raw: string): Line {
  const text = raw.trim();
  if (!text) throw new Error("নিয়ামক রেখার সমীকরণ লিখুন");
  ...
  if (Math.abs(c.A) > EPS || Math.abs(c.B) > EPS || Math.abs(c.C) > EPS)
    throw new Error("নিয়ামক অবশ্যই সরলরেখা হতে হবে (x², xy, y² থাকতে পারবে না)");
  if (Math.abs(c.D) < EPS && Math.abs(c.E) < EPS)
    throw new Error("রেখার সমীকরণে x বা y থাকতে হবে");
...
function fromFocusDirectrix(focus: Pt, dir: Line) {
  ...
  if (Math.abs(sd) < 1e-8)
    throw new Error("উপকেন্দ্র নিয়ামক রেখার উপরে আছে — পরাবৃত্ত গঠিত হয় না");
  const sgn = sd > 0 ? 1 : -1;
...
function buildCore(focus: Pt, dir: Line): AdvResult {
  ...
  const standardTex = rotated
    ? `Y^2 = ${fmt(4 * a)}X \\quad (\\text{ঘূর্ণিত অক্ষ } X, Y)`
...
function derivationSteps(focus: Pt, dir: Line, res: AdvResult): StepBlock[] {
  ...
  steps.push({
    title: "ধাপ ১ — সংজ্ঞা প্রয়োগ",
    note: "পরাবৃত্তের সংজ্ঞা: উপকেন্দ্র S থেকে দূরত্ব = নিয়ামক রেখা থেকে লম্ব দূরত্ব, অর্থাৎ উৎকেন্দ্রিকতা e = 1।",
    lines: [
      `S = (${fmt(al)},\\; ${fmt(be)}), \\qquad \\text{নিয়ামক } M: ${lineTex(dir)}`,
  ...
  steps.push({
    title: "ধাপ ২ — দূরত্ব দুটির রাশি",
    note: "দুই বিন্দুর দূরত্ব ও বিন্দু-রেখা লম্ব দূরত্বের সূত্র বসানো হলো।",
    lines: [
  ...
  steps.push({
    title: "ধাপ ৩ — মান বসানো",
    note: "α, β এবং a, b, c এর প্রদত্ত মান বসানো হলো।",
    lines: [
  ...
  steps.push({
    title: "ধাপ ৪ — হর দূর করা (আড়াআড়ি গুণ)",
    note: `উভয় পক্ষকে a² + b² = ${fmt(k)} দিয়ে গুণ করা হলো।`,
    lines: [
  ...
  steps.push({
    title: "ধাপ ৫ — বামপক্ষের বিস্তার",
    note: "(p − q)² = p² − 2pq + q² সূত্র ব্যবহার করা হলো।",
    lines: [
      `(x ${signed(-al)})^2 = ${lx}`,
      `(y ${signed(-be)})^2 = ${ly}`,
      `\\text{বামপক্ষ} = ${fmt(k)}\\left[${lx} + ${ly}\\right]`,
  ...
  steps.push({
    title: "ধাপ ৬ — ডানপক্ষের বিস্তার",
    note: "(p + q + r)² = p² + q² + r² + 2pq + 2qr + 2rp সূত্র ব্যবহার করা হলো।",
    lines: [
  ...
  steps.push({
    title: "ধাপ ৭ — সব পদ বামপক্ষে এনে সমচিহ্ন পদ একত্র",
    note: "প্রতিটি সহগ আলাদাভাবে হিসাব করা হলো, কোনো ধাপ বাদ নেই।",
    lines: [
      `x^2 \\text{ এর সহগ} = ${fmt(k)} - ${fmt(a * a)} = ${fmt(A)}`,
      `y^2 \\text{ এর সহগ} = ${fmt(k)} - ${fmt(b * b)} = ${fmt(C)}`,
      `xy \\text{ এর সহগ} = 0 - ${fmt(2 * a * b)} = ${fmt(B)}`,
      `x \\text{ এর সহগ} = ${fmt(-2 * al * k)} - ${fmt(2 * a * c)} = ${fmt(D)}`,
      `y \\text{ এর সহগ} = ${fmt(-2 * be * k)} - ${fmt(2 * b * c)} = ${fmt(E)}`,
      `\\text{ধ্রুবপদ} = ${fmt(k * (al * al + be * be))} - ${fmt(c * c)} = ${fmt(F)}`,
    ],
  });
  ...
  const lines8 = [`${rawEq}`];
  if (reducedEq !== rawEq) lines8.push(`\\text{সাধারণ উৎপাদক দিয়ে ভাগ করে:}\\quad ${reducedEq}`);
  steps.push({
    title: "ধাপ ৮ — নির্ণেয় সাধারণ দ্বিঘাত সমীকরণ",
    note: "Ax² + Bxy + Cy² + Dx + Ey + F = 0 আকারে সাজানো হলো।",
    lines: lines8,
  });

  const disc = res.coefs.B * res.coefs.B - 4 * res.coefs.A * res.coefs.C;
  steps.push({
    title: "ধাপ ৯ — যাচাই (শর্ত B² − 4AC = 0)",
    note: "পরাবৃত্তের জন্য নিরূপক শূন্য হওয়া আবশ্যক।",
    lines: [
      `B^2 - 4AC = (${fmt(res.coefs.B)})^2 - 4(${fmt(res.coefs.A)})(${fmt(res.coefs.C)}) = ${fmt(disc)}`,
      `\\therefore \\text{লেখচিত্রটি একটি পরাবৃত্ত}`,
    ],
  });
...
function propertySteps(res: AdvResult): StepBlock[] {
  ...
  const steps: StepBlock[] = [];
  steps.push({
    title: "ধাপ ১০ — শীর্ষবিন্দু ও অক্ষরেখা",
    note: "শীর্ষ হলো উপকেন্দ্র ও নিয়ামকের মধ্যবিন্দু; অক্ষরেখা নিয়ামকের উপর লম্ব এবং উপকেন্দ্রগামী।",
    lines: [
      ...
      `\\text{অক্ষরেখা}: ${lineTex(res.axis)}`,
    ],
  });
  steps.push({
    title: "ধাপ ১১ — উপকেন্দ্রিক লম্ব (Latus Rectum)",
    note: "উপকেন্দ্রগামী, অক্ষের উপর লম্ব জ্যা-এর দৈর্ঘ্য 4a।",
    lines: [
      `\\text{দৈর্ঘ্য} = 4a = 4 \\times ${fmt(a)} = ${fmt(4 * a)}`,
      `L_1 = (${fmt(latusEnds[0][0])},\\; ${fmt(latusEnds[0][1])}), \\qquad L_2 = (${fmt(latusEnds[1][0])},\\; ${fmt(latusEnds[1][1])})`,
      `\\text{উপকেন্দ্রিক লম্বের সমীকরণ}: ${lineTex({ a: d.a, b: d.b, c: -(d.a * focus[0] + d.b * focus[1]) })}`,
    ],
  });
  steps.push({
    title: "ধাপ ১২ — উপকেন্দ্রিক দূরত্ব",
    note: "যেকোনো বিন্দু P(x, y) এর উপকেন্দ্রিক দূরত্ব = নিয়ামক থেকে তার লম্ব দূরত্ব।",
    lines: [
      `SP = PM = \\dfrac{|${fmt(d.a)}x ${signed(d.b)}y ${signed(d.c)}|}{${sqrtTex(d.a * d.a + d.b * d.b)}}`,
      `\\text{শীর্ষে } SP = a = ${fmt(a)}`,
    ],
  });
  return steps;
}
...
function facts(res: AdvResult): Fact[] {
  ...
  return [
    { label: "উপকেন্দ্র S", tex: `(${fmt(focus[0])},\\; ${fmt(focus[1])})`, color: "#f59e0b" },
    { label: "শীর্ষবিন্দু A", tex: `(${fmt(vertex[0])},\\; ${fmt(vertex[1])})`, color: "#ef4444" },
    { label: "নিয়ামক রেখা", tex: lineTex(res.directrix), color: "#22c55e" },
    { label: "অক্ষরেখা", tex: lineTex(res.axis), color: "#0ea5e9" },
    { label: "উপকেন্দ্রিক দূরত্ব a", tex: fmt(a), color: "#a78bfa" },
    { label: "উপকেন্দ্রিক লম্ব", tex: `4a = ${fmt(4 * a)}`, color: "#d946ef" },
    {
      label: "লম্বের প্রান্তবিন্দু",
      tex: `(${fmt(latusEnds[0][0])},\\; ${fmt(latusEnds[0][1])}),\\; (${fmt(latusEnds[1][0])},\\; ${fmt(latusEnds[1][1])})`,
      color: "#ec4899",
    },
    { label: "উৎকেন্দ্রিকতা e", tex: "1", color: "#29d3b0" },
    { label: "ঘূর্ণন কোণ", tex: `${fmt(res.rotationDeg)}^\\circ`, color: "#48a9ff" },
  ];
}
...
function standardFormSteps(c: PolyCoefs): { steps: StepBlock[]; focus: Pt; directrix: Line } {
  ...
  steps.push({
    title: "ধাপ ১ — সমীকরণের প্রকৃতি যাচাই",
    note: "B² − 4AC = 0 হলে বক্ররেখাটি পরাবৃত্ত।",
    lines: [
      `${conicTex(c)}`,
  ...
  if (Math.abs(disc) > 1e-6) throw new Error("এই সমীকরণটি পরাবৃত্ত নয় (B² − 4AC ≠ 0)");
  if (Math.abs(c.B) > 1e-8)
    throw new Error("ঘূর্ণিত সাধারণ সমীকরণের জন্য Type B/C/D ব্যবহার করুন");

  if (Math.abs(c.A) > 1e-8 && Math.abs(c.C) < 1e-8) {
    if (Math.abs(c.E) < 1e-8) throw new Error("এটি পরাবৃত্ত নয়, সরলরেখা যুগল");
    const A = c.A;
    const h = -c.D / (2 * A);
    steps.push({
      title: "ধাপ ২ — x পদগুলোর পূর্ণবর্গ করা",
      note: "x² এর সহগ দিয়ে ভাগ করে পূর্ণবর্গ গঠন করা হলো।",
      lines: [
    ...
    steps.push({
      title: "ধাপ ৩ — আদর্শ আকার",
      note: "(x − h)² = 4a(y − k) আকারে সাজানো হলো।",
      lines: [
  ...
  if (Math.abs(c.C) > 1e-8 && Math.abs(c.A) < 1e-8) {
    if (Math.abs(c.D) < 1e-8) throw new Error("এটি পরাবৃত্ত নয়, সরলরেখা যুগল");
    const C = c.C;
    const kk = -c.E / (2 * C);
    steps.push({
      title: "ধাপ ২ — y পদগুলোর পূর্ণবর্গ করা",
      note: "y² এর সহগ দিয়ে ভাগ করে পূর্ণবর্গ গঠন করা হলো।",
      lines: [
    ...
    steps.push({
      title: "ধাপ ৩ — আদর্শ আকার",
      note: "(y − k)² = 4a(x − h) আকারে সাজানো হলো।",
      lines: [
  ...
  throw new Error("সমীকরণে x² অথবা y² যেকোনো একটি থাকতে হবে");
}

const BN = "০১২৩৪৫৬৭৮৯";
const bnNum = (n: number) => String(n).split("").map((d) => BN[Number(d)]).join("");

function renumber(steps: StepBlock[]): StepBlock[] {
  return steps.map((s, i) => ({
    ...s,
    title: s.title.replace(/^(ধাপ|যাচাই ধাপ)\s*[০-৯0-9]+/, `ধাপ ${bnNum(i + 1)}`),
  }));
}
...
export function solveAdvanced(input: AdvInput): AdvResult {
  if (input.kind === "A") {
    ...
    if ([coefs.A, coefs.B, coefs.C].every((v) => Math.abs(v) < 1e-10))
      throw new Error("এটি দ্বিঘাত সমীকরণ নয়");
  ...
  if (input.kind === "C") {
    ...
    if (Math.abs(sd) < 1e-8) throw new Error("শীর্ষবিন্দু নিয়ামক রেখার উপরে থাকতে পারে না");
    const sgn = sd > 0 ? 1 : -1;
    ...
    const pre: StepBlock = {
      title: "ধাপ ০ — শীর্ষ ও নিয়ামক থেকে উপকেন্দ্র নির্ণয়",
      note: "শীর্ষ নিয়ামক ও উপকেন্দ্রের ঠিক মধ্যবিন্দু, তাই উপকেন্দ্র = শীর্ষ থেকে সমান দূরত্বে নিয়ামকের বিপরীত দিকে।",
      lines: [
  ...
  if (a < 1e-8) throw new Error("উপকেন্দ্র ও শীর্ষবিন্দু একই বিন্দু হতে পারে না");
  ...
  const pre: StepBlock = {
    title: "ধাপ ০ — উপকেন্দ্র ও শীর্ষ থেকে নিয়ামক নির্ণয়",
    note: "নিয়ামক অক্ষরেখার উপর লম্ব এবং শীর্ষ থেকে a দূরত্বে উপকেন্দ্রের বিপরীত পাশে।",
    lines: [
      ...
      `Q = A - a\\hat{u} = (${fmt(Q[0])},\\; ${fmt(Q[1])}) \\quad \\text{(নিয়ামকের পাদবিন্দু)}`,
      `\\text{নিয়ামক}: ${lineTex(d)}`,
    ],
  };
