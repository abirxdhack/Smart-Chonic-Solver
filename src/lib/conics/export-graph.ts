...
export async function downloadGraphPng(container: HTMLElement | null, filename: string) {
  ...
  if (!svg) throw new Error(trs("গ্রাফ পাওয়া যায়নি"));
  ...
  await new Promise<void>((resolve, reject) => {
    img.onload = () => resolve();
    img.onerror = () => reject(new Error(trs("ছবি তৈরি করা যায়নি")));
    img.src = src;
  });
  const scale = 2;
  ...
  if (!ctx) throw new Error(trs("ক্যানভাস সমর্থিত নয়"));
  ctx.fillStyle = "#ffffff";
  ctx.fillRect(0, 0, canvas.width, canvas.height);
  ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
  const blob: Blob | null = await new Promise((resolve) => canvas.toBlob(resolve, "image/png"));
  if (!blob) throw new Error(trs("ডাউনলোড ব্যর্থ"));
